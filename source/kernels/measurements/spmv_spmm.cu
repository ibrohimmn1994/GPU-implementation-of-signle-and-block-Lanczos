...
#include "../../methods/copy_functions.hpp"
...
int main(int argc, char **argv)
{
	...
	auto info = Matrix_A<float>(N,N,N);
        Ell_matrix<float> D_host = info.first; // D = [0 Dh; De 0]
        Ell_matrix<float> W_host = info.second;
	D_host.mult_diagonal(W_host);
	D_host.change_order(4);
	Ell_matrix<float> A = D_host.copy_to_device();
	...
	if (0)
	{
		Dense_matrix<float> B_host = matrix_B<float>( N, A.n_rows());
	        Dense_matrix<float> B = B_host.copy_to_device();
       	 	Dense_matrix<float> R(B);
		...
		std::cout << "rows " << A.n_rows() << std::endl;
        	...
        	std::cout << "BW " << 1e-9*( (A.memory_consumption() + 2*B.memory_consumption() ))/best<< std::endl;
        	std::cout << " GFLPs " <<1e-9*( ( 2*A.n_rows()*A.width()*B.n_cols()  )  )/best << std::endl;
		
	}
	else
	{
		Vector<float> b_host = vector_b<float>( N, A.n_rows());
		Vector<float> b = b_host.copy_to_device();
		Vector<float> r(b);
		...
		std::cout << "rows " << A.n_rows() << std::endl;
        	std::cout << "elapsed time: " << std::setw(11) << best << std::endl;
        	std::cout << "BW " << 1e-9*( (A.memory_consumption() + 2*b.memory_consumption() ))/best<< std::endl;
        	std::cout << " GFLPs " <<1e-9*( ( 2*A.n_rows()*A.width()  )  )/best << std::endl;
	}
