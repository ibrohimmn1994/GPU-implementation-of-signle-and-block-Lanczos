...
int main(int argc, char **argv)
{
	...
	Dense_matrix<float> B_host = matrix_B<float>( N, D_host.n_rows());
	Dense_matrix<float> B = B_host.copy_to_device();

	Dense_matrix<float> R(N_COL, N_COL, mem_cuda);
	Dense_matrix<float> temp(R); 

	cublasHandle_t cublasH;
        CUBLAS_CHECK( cublasCreate(&cublasH) );
	mm_tt_cublas( B, temp, cublasH);                       
		...
		for (unsigned int t = 0; t<n_test; ++t)
                {
                        ...
                        {
				 my_sqrtm_cusolver<float>(R, temp);
                        }
	...
	{
		cusolver_args<float> args = cusolver_args<float>();
                Vector<float> eigen_val(N_COL, mem_cuda);
                initiate_cusolver(args, R, eigen_val);
		...
		for (unsigned int t = 0; t<n_test; ++t)
			...
			cudaDeviceSynchronize();
                	time.start();
                        for (rep = 0; rep < n_rep ; ++ rep)
                        {
				sqrtm_cusolver(eigen_val, R, temp, args);
                        }
