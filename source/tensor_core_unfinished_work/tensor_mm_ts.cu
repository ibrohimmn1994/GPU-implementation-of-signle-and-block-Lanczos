...
__global__ void tc_mm_ts1(	const unsigned int global_n_rows,
			 	float *T,
				float *S
				float*result)
{
	...
	wmma::fragment<wmma::matrix_a, TILE_SIZE, TILE_SIZE, TILE_SIZE, half, wmma::row_major>t;
	wmma::fragment<wmma::matrix_b, TILE_SIZE, TILE_SIZE, TILE_SIZE, half, wmma::col_major>s;

	//check this also need to convert into half and then load
	// 	__float2half
	// cuda_fp16.h#include "cuda_fp16.h"
	//#include "device_functions.h"
	/////////////////////////////////////////////////////////////////
	 float *shmem_s_ptr = (float *)&
			shmem[ (warpId% WARPS_PER_BLOCK)*2 + laneId/(WARP_SIZE/2) ][0] 
			+ laneId % (WARP_SIZE/2);
	*shmem_s_ptr = s[laneId + warpId * WARP_SIZE ];

	wmma::load_matrix_sync(s, &(shmem[0][0]), TILE_SIZE);
	
	//or but problem it is float
	 //wmma::load_matrix_sync(s, &s[0], TILE_SIZE);

	__syncthreads();
	//////////////////////////////////////////////////////////////
	half *warp_ptr =  &T[blockIdx.x * TILES_PER_BLOCK * TILE_SIZE] +
